...
function simplifyBill(bill, meta, sessionInfo) {
  ...
  return {
    ...
    template_url: meta.template_url || null,
...
function errorEntry(state, entry, statusLabel, errorMsg) {
  return {
    ...
    template_url: entry.template_url || null,
...
// Passes a curated entry straight through to bills.json with no LegiScan
// call at all. Used for advocacy "campaigns" that aren't an introduced
// bill LegiScan can look up — e.g. asking legislators to introduce a
// resolution. These never reach a FINAL_STATUSES status and have no
// final_date, so they are never auto-expired; they stay listed until
// someone removes them from tracked-bills.json by hand.
function staticEntry(state, entry) {
  return {
    state,
    bill_number: entry.bill_number,
    label: entry.label || null,
    ...
    template_url: entry.template_url || null,
    chamber_target: entry.chamber_target || 'both',
    position: entry.position || 'support',
    position_notes: entry.position_notes || '',
    email_subject: entry.email_subject || null,
    email_template: entry.email_template || null,
    title: entry.label || null,
    status_label: entry.status_label_override || 'Ongoing',
    status_note: null,
...
async function main() {
  ...
  for (const [state, entries] of Object.entries(byState)) {
    console.log(`Processing state: ${state || "(none — nationwide campaign)"} (${entries.length} bill(s))`);

    // Handle skip_legiscan entries (advocacy campaigns, not real
    // introduced bills — e.g. a nationwide resolution ask with no
    // state at all) before touching the LegiScan API. This matters
    // because a group made up entirely of skip_legiscan entries (like
    // an empty/blank "state") has nothing for buildBillIdMap to look
    // up, and would otherwise incorrectly turn every entry in the
    // group into an "Error" result.
    const skipEntries = entries.filter((e) => e.skip_legiscan);
    const legiscanEntries = entries.filter((e) => !e.skip_legiscan);

    for (const entry of skipEntries) {
      console.log(`  [${state || "—"}] ${entry.bill_number} — skip_legiscan=true, passing through without a LegiScan lookup`);
      results.push(staticEntry(state, entry));
    }

    if (!legiscanEntries.length) continue;
    ...
    try {
      ...
      for (const entry of legiscanEntries) {
