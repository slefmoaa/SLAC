...
- `bill_number` — must match LegiScan's format (e.g. `HB123`, `SB45`, no space).
  **Nebraska is the exception** — LegiScan uses the `LB###` (Legislative
  Bill) prefix instead of `HB`/`SB`, since the Unicameral has no separate
  House or Senate.
...
- `chamber_target` — `"lower"` (House/Assembly), `"upper"` (Senate),
  `"both"`, or `"unicameral"` (Nebraska only — the Unicameral has a single
  chamber, so there's no House/Senate distinction to filter on). Determines
  which legislators are shown on the take-action page; `"unicameral"` is
  treated the same as `"both"` and simply accepts whichever legislator
  Open States returns for the address (in practice, exactly one).
...
| `bill_number` | string | ✓ | LegiScan-format bill number (`LB###` for Nebraska) |
...
| `chamber_target` | string | ✓ | `"lower"`, `"upper"`, `"both"`, or `"unicameral"` (Nebraska only) |
...
- **Open States** — free tier handles per-user take-action lookups
  ...
  boundaries. Note that Nebraska's legislators are classified differently
  from other states' (there's no `upper`/`lower` split), which is why
  `chamber_target: "unicameral"` exists as a separate value rather than
  reusing `"both"`.
